 ...
 *
 */
public class JedisUtilBak {
	public static Properties properties = new Properties();
	private static JedisPool jedisPool = null;
	
	static{
		InputStream is = JedisUtilBak.class.getClassLoader().getResourceAsStream("config/system.properties");
		try {
			properties.load(is);
			is.close();
		} catch (Exception e) {
	...
	private static synchronized JedisPool getJedisPool(){
		if(jedisPool==null){
			...
			String host = properties.getProperty("redisHost_bak");
			String port = properties.getProperty("redisPort_bak");
			String password = properties.getProperty("redisPassword_bak");//instance_id:password
...
//	}
	...
	public static void main(String[] args) {
		try{
			JedisUtilBak.set("foo","test");
			String str = JedisUtilBak.get("foo");
			System.out.println("获取缓存数据成功------"+str);
			
//			String stockcodelist = JedisUtilBak.get(StockConstant.KEY_STOCK_CODE);
...
//				}
//				
//				List<String> minuteBarList = JedisUtilBak.getAll(key);
//				if(minuteBarList!=null && minuteBarList.size()>0){
//					JedisUtilBak.del(key);
//					for(String minuteBar:minuteBarList){
//						JedisUtilBak.push(key, minuteBar);
//					}
//
//					JedisUtilBak.del(key);
